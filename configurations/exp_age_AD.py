...
EXPERIMENT_PARAMS = {
    ...
    "folder": "experiments/method_age_latent_space_%d_%s_age_ord_%s_%s_"%(latent_space,age_dim, age_ord, Norm)+t.split(' ')[0]+'-'+t.split(' ')[1].replace('.','-').replace(':','-'),  # Folder to store experiment results.
    "epochs": 600,
    "batch_size": 32,
    "split": 0,  # Default split of the dataset.
    "augment": True,  # Data augmentation
    "model": "synthesis_model_age.synthesis_model_age",  # Selected GAN architecture.
    "executor": "executor_age.executor_age",  # Selected experiment executor.
    "out_channels": 1,
    "gp_weight":10,
    "outputs": 1,
    "filters":32,
