...
class critic_2D(BaseNet):
    def __init__(self, conf):
        super(critic_2D, self).__init__(conf)
    ...
    def build(self, _subname = None):
        ...
        # =============================================================================================================
        # Encode age in the latent space
        age_vector = Input(shape=(self.conf.age_dim,))
        # (batch size, 10, 13, filters*8)
        mid1_1 = conv2D_layer(pool4, name="mid1_1", filters=32, kernel_size=3, strides=1, padding='same',
                                 activation='relu', kernel_initializer="he_normal")
        # (batch size, 10, 13, 32)
        ...
        # =============================================================================================================
        conv5_1 = conv2D_layer(mid_concat2_1, name="conv5_1", filters=f*16, kernel_size=3, strides=1, padding='same',
                                  activation='relu', kernel_initializer="he_normal" )
        ...
        self.model = Model([d_input, age_vector], averagePool, name=name)
    def compile(self):
